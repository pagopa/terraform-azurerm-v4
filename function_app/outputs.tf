output "id" {
  value = azurerm_linux_function_app.this.id
}

output "resource_group_name" {
  value = azurerm_linux_function_app.this.resource_group_name
}

output "name" {
  value = var.name
}

output "default_hostname" {
  value     = azurerm_linux_function_app.this.default_hostname
  sensitive = true
}

output "possible_outbound_ip_addresses" {
  value = azurerm_linux_function_app.this.possible_outbound_ip_addresses
}

output "default_key" {
  value     = var.export_keys ? data.azurerm_function_app_host_keys.this[0].default_function_key : null
  sensitive = true
}

output "master_key" {
  value     = var.export_keys ? data.azurerm_function_app_host_keys.this[0].primary_key : null
  sensitive = true
}

output "primary_key" {
  value     = var.export_keys ? data.azurerm_function_app_host_keys.this[0].primary_key : null
  sensitive = true
}

output "app_service_plan_id" {
  value = var.app_service_plan_id != null ? var.app_service_plan_id : azurerm_service_plan.this[0].id
}

output "app_service_plan_name" {
  value = var.app_service_plan_id != null ? var.app_service_plan_name : azurerm_service_plan.this[0].name
}

output "storage_account_name" {
  value = var.default_storage_enable ? module.storage_account[0].name : var.storage_account_name
}

output "storage_account" {
  value = var.default_storage_enable ? {
    name                      = module.storage_account[0].name
    primary_access_key        = module.storage_account[0].primary_access_key
    primary_connection_string = module.storage_account[0].primary_connection_string
    } : var.storage_account_name != null ? {
    name                      = var.storage_account_name
    primary_access_key        = var.storage_account_access_key
    primary_connection_string = null
  } : null
  sensitive = true
}

output "storage_account_internal_function_name" {
  value       = var.internal_storage.enable ? module.storage_account_durable_function[0].name : null
  description = "Storage account used by the function for internal operations."
}

output "storage_account_internal_function" {
  value = var.internal_storage.enable ? {
    name                      = module.storage_account_durable_function[0].name
    primary_access_key        = module.storage_account_durable_function[0].primary_access_key
    primary_connection_string = module.storage_account_durable_function[0].primary_connection_string
  } : null
  sensitive   = true
  description = "Storage account used by the function for internal operations."
}

output "system_identity_principal" {
  value       = var.system_identity_enabled ? azurerm_linux_function_app.this.identity[0].principal_id : null
  description = "Service Principal of the System Identity generated by Azure."
}

output "user_identity_principal" {
  value       = length(var.user_identity_ids) > 0 ? azurerm_linux_function_app.this.identity[0].identity_ids : null
  description = "A list of User Assigned Managed Identity IDs assigned to this Function App."
}
