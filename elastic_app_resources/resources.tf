...
resource "elasticstack_kibana_alerting_rule" "alert" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = lookup(each.value, "log_query", null) != null && contains(["sum", "avg", "min", "max"], try(each.value.log_query.aggregation.type, null)) ? try(each.value.log_query.aggregation.field, null) != null : true
