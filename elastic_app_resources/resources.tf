...
resource "elasticstack_kibana_alerting_rule" "alert" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = can(each.value.custom_threshold) ? alltrue([for agg in each.value.custom_threshold.aggregations: contains(local.allowed_aggregations, try(agg.aggregation, null))]) : true
