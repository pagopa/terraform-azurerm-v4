...
resource "time_rotating" "cert_rotation" {
  ...
  rotation_days = each.value.validity_in_months * 30 - var.renewal_days_before_expiry
}
...
resource "time_rotating" "cert_stable" {
  for_each      = var.certificates
  rotation_days = each.value.validity_in_months * 30 - var.stable_promotion_days_before_expiry
}
