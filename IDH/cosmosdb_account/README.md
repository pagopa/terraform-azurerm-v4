...
| <a name="input_consistency_level_override"></a> [consistency\_level\_override](#input\_consistency\_level\_override) | (Optional) Consistency level for the CosmosDB account. Allowed values are: 'Eventual', 'Session', 'BoundedStaleness', 'Strong', 'ConsistentPrefix'. Uses library default if not provided | `string` | `null` | no |
