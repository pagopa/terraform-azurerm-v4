resource "azurerm_data_factory_managed_private_endpoint" "df_connection_managed_private_endpoint" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = length([for k, v in var.adf_managed_private_endpoint : k if v.type == "postgres"]) <= 1
      error_message = "Only one element in var.adf_managed_private_endpoint can have type 'postgres'. Multiple linked services must share the same private endpoint"
    }

    precondition {
      condition     = alltrue([for k, v in var.adf_managed_private_endpoint : contains(keys(local.az_api_type_mappings), v.type)])
      error_message = "All elements in var.adf_managed_private_endpoint must have a 'type' that is a valid key in local.az_api_type_mappings."
    }
  }
}
...
locals {
  ...
  connection_to_approve = {
    for key, value in var.adf_managed_private_endpoint :
    key => [
