...
| <a name="input_enabled_only_rules"></a> [enabled\_only\_rules](#input\_enabled\_only\_rules) | (Optional) Enables a 'rules-only' mode. If 'enabled' is set to true, the module skips the creation of the NSG, association, and flow logs | <pre>object({<br/>    enabled             = bool<br/>    security_group_name = optional(string, null)<br/>  })</pre> | <pre>{<br/>  "enabled": false,<br/>  "security_group_name": null<br/>}</pre> | no |
