...
resource "grafana_folder" "domainsfolder" {
  provider = grafana.cloud
  for_each = { for i in range(length(local.dashboard_resource_map)) : join("-",toset(local.dashboard_resource_map[i].domain_exists),toset(split("/", local.dashboard_resource_map[i].type)[1])) => i  }
