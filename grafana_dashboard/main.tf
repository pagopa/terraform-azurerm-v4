...
resource "grafana_folder" "domainsfolder" {
  provider = grafana.cloud
  for_each = { for i in range(length(local.dashboard_resource_map)) : merge(local.dashboard_resource_map[i].type,split("/", local.dashboard_resource_map[each.value].type)[1]) => i  }
